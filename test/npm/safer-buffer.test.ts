...
describe(`npm > ${regPkgName}`, async () => {
  ...
  it('.prototype property of Buffer is inherited', () => {
    for (const impl of implementations) {
      assert.equal(impl.Buffer.prototype, buffer.Buffer.prototype, 'prototype')
  ...
  it('Constructor is buffer.Buffer', () => {
    ...
    for (const arg of [0, 10, 100]) {
      assert.equal(dangerous.Buffer.allocUnsafe(arg).constructor, buffer.Buffer)
      assert.equal(
  ...
  it('.alloc(size, fill) is `fill`-filled', () => {
    ...
    if (tmp[1] === tmp[0]) {
      // Outdated Node.js
      assert.deepEqual(index.Buffer.alloc(5, 'ok'), index.Buffer.from('ooooo'))
    } else {
      assert.deepEqual(index.Buffer.alloc(5, 'ok'), index.Buffer.from('okoko'))
    }
    assert.notDeepEqual(index.Buffer.alloc(5, 'ok'), index.Buffer.from('kokok'))
  })

  it('safer.Buffer.from returns results same as Buffer constructor', () => {
    for (const impl of implementations) {
      assert.deepEqual(impl.Buffer.from(''), new buffer.Buffer(''))
      assert.deepEqual(impl.Buffer.from('string'), new buffer.Buffer('string'))
      assert.deepEqual(
