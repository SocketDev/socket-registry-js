...
function fmt_resolution(a) {
  ...
  if (
    ...
    let v = str(new Uint8Array(view2.buffer, view2.byteOffset + pos, 8))
    ...
    let out =
      tag === 32 /* git */
        ...
        : tag === 16 /* github */
          ? 'github:'
          : 'gitlab:'
    let owner = str(new Uint8Array(view2.buffer, view2.byteOffset + pos, 8))
    let repo = str(new Uint8Array(view2.buffer, view2.byteOffset + pos + 8, 8))
    if (owner) out += owner + '/'
    else if (is_scp(repo)) out += 'ssh://'
    out += repo
    pos += 16
    let commitish = str(new Uint8Array(view2.buffer, view2.byteOffset + pos, 8))
    let resolved = str(
...
function fmt_url(a) {
  if (a.byteLength < 64) throw new TypeError('resolution too short')
  return a[0] === 2 /* npm */
    ? str(new Uint8Array(a.buffer, a.byteOffset + 8, 8))
    : fmt_resolution(a)
}
